...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        select_keys = ['uid', 'responses', 'input_ids', 'attention_mask', 'position_ids', 'old_log_probs', 'advantages', 'seq_level_rewards']
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                if self.config.use_dynamic_bsz:
                    if self.config.use_max_seq_len:
                        micro_bsz = self.config.ppo_max_token_len_per_gpu//self.config.max_seq_len
                ...
                for data in micro_batches:
                    ...
                    tau = self.config.tau
                    kl_type=self.config.ppo_kl_type
                    ...
                    if self.config.loss_type == 'grpo':
                        ...
                        pg_loss, pg_clipfrac, ppo_kl = core_algos.compute_policy_loss_disco_logL(old_log_prob=old_log_prob,
                                                                                    ...
                                                                                    tau=tau,
                                                                                    kl_type=kl_type)
                    elif self.config.loss_type == 'drpo':
                        pg_loss, pg_clipfrac, ppo_kl = core_algos.compute_policy_loss_drpo(old_log_prob=old_log_prob,
                                                                                    log_prob=log_prob,
                                                                                    eos_mask=response_mask,
                                                                                    uid=uid,
                                                                                    seq_level_rewards=seq_level_rewards,
